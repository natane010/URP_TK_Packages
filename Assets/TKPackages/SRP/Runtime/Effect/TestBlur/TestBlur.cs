...
{
     [VolumeComponentMenu("(Bokeh Blur)")]
    public class BokehBlur : PostProcessSettingsBase
    {
    ...
    public class BokehBlurRenderer : PostProcessRendererBase<BokehBlur>
    {
        ...
        public override void Render(CommandBuffer cmd, RTHandle source, RTHandle target,
            ref RenderingData renderingData)
        {
            ...
            desc.sRGB = true;
            RenderingUtils.ReAllocateIfNeeded(ref m_BufferRT1, desc, FilterMode.Bilinear, TextureWrapMode.Clamp,
                name: ShaderIDs.BufferRT1);

            Blitter.BlitCameraTexture(cmd, source, m_BufferRT1);
            m_BlitMaterial.material.SetVector(ShaderIDs.GoldenRot, m_GoldenRot);
            m_BlitMaterial.material.SetVector(ShaderIDs.Params,
                new Vector4(m_Settings.Iteration.value, m_Settings.BlurRadius.value, 1f / Screen.width,
                    1f / Screen.height));
            Blitter.BlitCameraTexture(cmd, m_BufferRT1, target, m_BlitMaterial.material, 0);
        }
    }
}
