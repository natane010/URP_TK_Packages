...
{
    public abstract class PostProcessPassBase : ScriptableRenderPass
    {
        ...
        public void AddEffects(Assembly assembly)
        {
            ...
            foreach (var type in types)
            {
                ...
                {
                    Debug.LogError($"Type is not interface: {type.FullName}");
                    continue;
                }
                ...
                {
                    mark = new VolumeRendererMark() { state = VolumeRendererState.WaitingToAdd, renderer = Activator.CreateInstance(type) as IPostProcessRenderer };
